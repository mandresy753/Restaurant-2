...
public class DataRetriever {
            ...
            PreparedStatement preparedStatement = connection.prepareStatement(
                    """
                            select dish.id as dish_id, dish.name as dish_name, 
                                   dish_type, dish.selling_price as dish_price
                            from dish
                            where dish.id = ?;
                            """);
            ...
            for (StockMovement stockMovement : stockMovementList) {
                if (ingredient.getId() != null) {
                    ...
                    preparedStatement.setInt(1,
                            getNextSerialValue(conn, "stock_movement", "id"));
                }
        ...
        try (Connection conn = new DBConnection().getConnection()) {
            ...
            if (toSave.getDishIngredients() != null) {
                for (DishIngredient di : toSave.getDishIngredients()) {
                    if (di.getDish() == null) di.setDish(toSave);
                }
    ...
    private void detachIngredients(Connection conn, List<DishIngredient> dishIngredients) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            for (Integer dishId : dishIngredientsGroupByDishId.keySet()) {
                ps.setInt(1, dishId);
                ps.addBatch();
            }
            ps.executeBatch(); // ⚡ suppression en batch
        }
    }
    ...
    private void attachIngredients(Connection conn, List<DishIngredient> ingredients)
        ...
        try (PreparedStatement ps = conn.prepareStatement(attachSql)) {
            for (DishIngredient dishIngredient : ingredients) {
                ...
                ps.setString(5, dishIngredient.getUnit().name()); // ⚡ Correction ici
                ps.addBatch();
            }
            ps.executeBatch();
        }
            ...
            PreparedStatement preparedStatement = connection.prepareStatement(
                    """
                            select ingredient.id, ingredient.name, ingredient.price, ingredient.category, 
                                   di.required_quantity, di.unit from ingredient join dish_ingredient di 
                                       on di.id_ingredient = ingredient.id where id_dish = ?;
                            """);
